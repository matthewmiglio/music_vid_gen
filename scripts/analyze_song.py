...
def analyze(path, length_arg="30", start_arg=None):
    ...
    if start_arg is not None:
        ...
        for t in downbeats:
            ...
            score = es[a:b].mean()
            in_win = [d["jump"] for d in drops if t + 2 <= d["t"] <= t + 0.6 * clip_len]
    ...
    inside = lambda t: clip_start <= t < clip_end
    clip = {
        ...
        "drops": [{**d, "t": rel(d["t"])} for d in drops if inside(d["t"])],
        "breakdowns": [{**d, "t": rel(d["t"])} for d in breakdowns if inside(d["t"])],
...
def main():
    ...
    song.update(bpm=a["tempo"]["bpm"], bar_seconds=a["tempo"]["bar_seconds"], key=a["key"]["key"],
                title=a["name"], source_start=c["start"])
